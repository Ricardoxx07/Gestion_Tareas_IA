"""Adaptador para Groq; el resto de la aplicación no depende de su API."""
...
class ProveedorGroq(ProveedorIAInterface):
    """Proveedor local que solicita recomendaciones estructuradas a Groq."""
    ...
    def recomendar_tareas(
        ...
        try:
            ...
            raise RecomendacionIAInvalidaError(
                "Groq no devolvió la estructura de planificación esperada"
            ) from exc
    ...
    def planificar_dia(
        ...
        try:
            ...
            raise RecomendacionIAInvalidaError(
                "Groq no devolvió la estructura de plan diario esperada"
            ) from exc
    ...
    def explicar_sobrecargas(
        ...
        try:
            ...
            raise RecomendacionIAInvalidaError(
                "Groq no devolvió la estructura de sobrecarga esperada"
            ) from exc
    ...
    def proponer_reprogramaciones(
        ...
        try:
            ...
            raise RecomendacionIAInvalidaError(
                "Groq no devolvió la estructura de reprogramación esperada"
            ) from exc
    ...
    def _validar_propuestas_reprogramacion(
        ...
        if contenido_limpio.startswith("```"):
            ...
            if len(lineas) < 3 or lineas[-1].strip() != "```":
                raise ValueError("El bloque JSON de Groq no está cerrado")
    ...
    def proponer_tareas(
        ...
        try:
            ...
            raise RecomendacionIAInvalidaError(
                "Groq no devolvió la estructura de propuestas esperada"
            ) from exc
    ...
    def descomponer_tarea(self, texto: str) -> DescomposicionTarea:
        ...
        try:
            ...
            raise RecomendacionIAInvalidaError(
                "Groq no devolvió la estructura de descomposición esperada"
            ) from exc
    ...
    def _normalizar_propuestas(
        ...
        if not ids_bloques_propuestos.issubset(bloques_por_id):
            raise RecomendacionIAInvalidaError(
                "Groq devolvió un bloque temporal fuera del contexto permitido"
            )
    ...
    def _solicitar_contenido(self, schema, mensajes: list[dict[str, str]]) -> str:
        try:
            ...
            logger.exception(
               "Falló una solicitud a Groq: %s",
               exc,
            )
            raise ProveedorIAError("No fue posible comunicarse con Groq") from exc
        ...
        contexto_tareas = [
            {
                ...
                "estado_fecha": ProveedorGroq._estado_fecha_plan(
                    tarea.fecha_limite,
                    fecha,
                ),
                "prioridad": tarea.prioridad,
                ...
                "tareas": [
                    {
                        ...
                        "estado_fecha": ProveedorGroq._estado_fecha_sobrecarga(
                            tarea.fecha_limite,
                            carga.fecha,
                        ),
                        "prioridad": tarea.prioridad,
    ...
    def _construir_mensajes_propuestas(
        ...
        return [
            ...
            },
            {
                ...
                "content": (
                    f"Fecha actual: {hoy.isoformat()}.\n"
                    f"Día actual: {ProveedorGroq.DIAS_SEMANA[hoy.weekday()]}.\n"
